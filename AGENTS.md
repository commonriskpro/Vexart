...
## Release & Conventional Commits Protocol

- **Automated Release via Google Release Please**: Vexart uses Google Release Please with unified versioning across the root and workspace packages.
- **Conventional Commits Mandate**: All commits and pull request titles MUST follow the Conventional Commits specification (`<type>(<scope>): <description>`).
  - Standard types: `feat(scope):`, `fix(scope):`, `perf(scope):`, `refactor(scope):`, `chore(scope):`, `docs(scope):`, `style(scope):`, `test(scope):`, `build(scope):`, `ci(scope):`, `revert(scope):`.
  - Breaking changes are designated with `!` before the colon (e.g. `feat(engine)!: break API`) or `BREAKING CHANGE:` in the commit footer.
  - Interactive/manual changeset creation (`bun run changeset`) is no longer required; release notes and semver version increments are generated automatically from Conventional Commits.
- **Native Rust Mapping**: The native crate `native/libvexart/` is consumed by `@vexart/engine` via FFI and is not published independently to npm. Changes to `native/libvexart/` must use scope `(engine)` or `(native)` (e.g. `fix(native): resolve shm buffer leak on macos` or `feat(engine): support wgpu multi-texture`).
- **Release Orchestration & Tagging**:
  - Merging PRs into `main` causes Google Release Please to update or create an automated release PR (`chore: release main`).
  - Merging the Release Please PR automatically tags `vX.Y.Z` on GitHub and publishes releases.
  - The `v*` tag trigger in `.github/workflows/build-native.yml` automatically initiates multi-platform native binary compilation for release distributions.
