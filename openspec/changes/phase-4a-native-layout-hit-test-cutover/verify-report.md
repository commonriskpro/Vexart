...
Implemented for the Phase 4a scope, with the retained interaction cutover path now live behind existing native flags.
...
- Native layout parity now covers multiline text sizing and floating/absolute positioning fixtures.
...
- `bun test packages/engine/src/ffi/native-scene.test.ts packages/engine/src/ffi/native-scene-layout-parity.test.ts packages/engine/src/loop/layout.test.ts` — 26 passed.
...
- Layout parity fixtures for nested scroll, border/padding combinations, multiline text sizing, and floating/absolute positioning.
