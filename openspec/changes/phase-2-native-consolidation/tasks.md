...
- [x] 5a.4 Same pattern: `paint/shaders/circle.wgsl` (bridge L683-738) + `paint/pipelines/circle.rs`. Vertex attributes per `CircleInstance` (16 floats).
- [x] 5a.5 Same: `paint/shaders/polygon.wgsl` (bridge L778-885) + `paint/pipelines/polygon.rs`. Vertex attributes per `PolygonInstance` (20 floats).
- [x] 5a.6 Same: `paint/shaders/bezier.wgsl` (bridge L889-1002) + `paint/pipelines/bezier.rs`. Vertex attributes per `BezierInstance` (20 floats).
- [x] 5a.7 Same: `paint/shaders/shape_rect.wgsl` (bridge L1006-1112) + `paint/pipelines/shape_rect.rs`. Vertex attributes per `ShapeRectInstance` (20 floats).
- [x] 5a.8 Same: `paint/shaders/rect_corners.wgsl` (bridge L1116-1231) + `paint/pipelines/rect_corners.rs`. Vertex attributes per `ShapeRectCornersInstance` (24 floats).
- [x] 5a.9 Same: `paint/shaders/glow.wgsl` (bridge L1491-1571) + `paint/pipelines/glow.rs`. Vertex attributes per `GlowInstance` (12 floats).
- [x] 5a.10 Same: `paint/shaders/nebula.wgsl` (bridge L1235-1380, complex 32-float instance with 4 gradient stops) + `paint/pipelines/nebula.rs`.
- [x] 5a.11 Same: `paint/shaders/starfield.wgsl` (bridge L1384-1487, 24-float instance with cluster + warm/neutral/cool colors) + `paint/pipelines/starfield.rs`.
- [x] 5a.12 Same: `paint/shaders/image.wgsl` (bridge L1575-1663) + `paint/pipelines/image.rs`. Uses `image_bind_group_layout` (texture + sampler bindings).
- [x] 5a.13 Same: `paint/shaders/image_transform.wgsl` (bridge L1757-1845) + `paint/pipelines/image_transform.rs`. Uses `image_bind_group_layout`.
- [x] 5a.14 Same: `paint/shaders/gradient_linear.wgsl` (bridge L1849-1949) + `paint/pipelines/gradient_linear.rs`. Vertex attributes per `LinearGradientInstance` (20 floats).
- [x] 5a.15 Same: `paint/shaders/gradient_radial.wgsl` (bridge L1953 to end of fn) + `paint/pipelines/gradient_radial.rs`. Vertex attributes per `RadialGradientInstance` (20 floats).
- [x] [ATOMIC] 5a.16 Wire `paint/mod.rs` `PaintContext::dispatch()` to (a) parse `graph_ptr` per design §8 (`parse_header()` returns `cmd_count` and `payload_bytes`), (b) iterate `cmd_count` command headers (cmd_kind + flags + payload_bytes), (c) for each command, deserialize payload into the appropriate Pod struct (`bytemuck::from_bytes`), (d) accumulate instance batches per pipeline (group by cmd_kind), (e) upload each batch as a `wgpu::Buffer` (vertex buffer with `Instance` step mode) via `device.create_buffer_init`, (f) submit one `wgpu::CommandEncoder` per frame with one render pass per pipeline that has instances, (g) write `gpu_ms` and `total_ms` into `*stats_out` if non-null. Use cmd_kind allocation per design §17.6 (0..10 reserved for the 13 ports; 11 reserved unused; 12-15 reserved for Slice 5b). Stub the dispatch for unknown cmd_kinds with a no-op (silent skip — Slice 5b will fill them).
- [x] [ATOMIC] 5a.17 Wire `vexart_paint_upload_image()` in `lib.rs` to create a `wgpu::Texture` from the input RGBA bytes (use `device.create_texture_with_data` from `wgpu::util::DeviceExt`), create a sampler, build an `wgpu::BindGroup` with the existing `image_bind_group_layout`, and store all 3 in an `Image` registry inside `PaintContext`. Return the registry handle as the `out_image` u64. Wire `vexart_paint_remove_image()` to drop the entry.
- [x] 5a.18 Add Rust unit test `native/libvexart/src/paint/pipelines/mod.rs` `#[cfg(test)] tests`: assert `PipelineRegistry::new(device, format, bgl)` returns OK when given a valid device. Use `pollster::block_on` to obtain a real adapter / device for the test (gated behind `#[cfg(feature = "gpu-tests")]`).
- [x] 5a.19 Add Rust unit test in `native/libvexart/src/paint/mod.rs`: build a graph buffer with 1 rect command (cmd_kind = 0), call `PaintContext::dispatch()` against a 64x64 offscreen target, verify it returns OK. Gated behind `#[cfg(feature = "gpu-tests")]`.
- [x] 5a.20 Verify: from repo root, `cargo check` passes with real WGPU code compiled. Then `cargo build` succeeds and produces `target/debug/libvexart.dylib`. Then `bun run typecheck` still passes (no TS imports yet of the new pipelines). The bridge.test.ts from Slice 4 still passes (no regression).
...
- [x] [ATOMIC] 5b.1 Create `paint/shaders/gradient_conic.wgsl` + `paint/pipelines/gradient_conic.rs` + `ConicGradientInstance` struct in `paint/instances.rs`. Algorithm: fragment shader computes `atan2(uv.y - center.y, uv.x - center.x)` normalized to [0, 1], samples a 2-stop or multi-stop color along that angle. Mirror the API shape of `LinearGradientInstance` with an extra `start_angle` field. Wire `cmd_kind = 14` in `paint/mod.rs` dispatch.
- [x] [ATOMIC] 5b.2 Create `paint/shaders/backdrop_blur.wgsl` + `paint/pipelines/backdrop_blur.rs` + `BackdropBlurInstance` struct (fields: rect [4 floats], blur_radius f32, padding). SIMPLIFICATION: single-pass 9-tap box blur (2-pass Gaussian deferred to Phase 2b — requires ping-pong textures). Replaces `apply_box_blur_rgba` (bridge L2427-2480). Wire `cmd_kind = 15` in `paint/mod.rs` dispatch.
- [x] [ATOMIC] 5b.3 Create `paint/shaders/backdrop_filter.wgsl` + `paint/pipelines/backdrop_filter.rs` + `BackdropFilterInstance` struct (fields: rect [4], brightness, contrast, saturate, grayscale, invert, sepia, hue_rotate_deg as 7 f32 fields + `_pad`). Single fragment shader applies 7 ops: brightness→contrast→saturate (luma blend)→grayscale→invert→sepia matrix→hue-rotate matrix. Coefficients verbatim from bridge L2497-2562. Replaces `apply_backdrop_filters_rgba` (bridge L2481-2579). Wire `cmd_kind = 16` in `paint/mod.rs` dispatch.
- [x] [ATOMIC] 5b.4 Create `paint/shaders/image_mask.wgsl` + `paint/pipelines/image_mask.rs` + `ImageMaskInstance` struct (fields: rect [4], mask_rect [4], radius_uniform, radius_tl, radius_tr, radius_br, radius_bl, mode, _pad0, _pad1 = 16 floats). Single fragment shader with SDF rounded-rect mask; mode branch (0.0=uniform, 1.0=per-corner). Replaces `apply_rounded_rect_mask_rgba` (bridge L2580-2633) AND `apply_rounded_rect_corners_mask_rgba` (bridge L2634-2699). Wire `cmd_kind = 17` in `paint/mod.rs` dispatch.
- [x] 5b.5 Update `PipelineRegistry` to hold the 4 new pipelines. Update its `new()` constructor to call the 4 new `create()` functions.
- [x] 5b.6 Add Rust unit test for gradient_conic: build a 32x32 offscreen target, dispatch 1 conic command 360°-spanning red→blue, sample center pixel — expect midpoint color (purple-ish). Gated `#[cfg(feature = "gpu-tests")]`.
- [x] 5b.7 Add Rust unit test for backdrop_blur: solid red 32x32 target, apply blur radius 4, verify edge pixels still red and center still red (uniform color stays uniform under blur). Gated `gpu-tests`.
- [x] 5b.8 Add Rust unit test for backdrop_filter: white 32x32 target, apply `brightness=50` (50%), verify all pixels mid-gray (0x80). Then test `invert=100` on white target → all pixels black. Gated `gpu-tests`.
- [x] 5b.9 Add Rust unit test for image_mask: red 64x64 image, mask with `radius_uniform=10` over center 40x40 rect, verify corner pixels of mask region are transparent (mask cuts the corners). Gated `gpu-tests`.
- [x] 5b.10 Verify: from repo root, `cargo check` passes. `cargo build` produces `libvexart.dylib`. `bun run typecheck` still passes. `cargo test --features gpu-tests` runs all 5a + 5b GPU tests green (29 passed).
...
- [x] 6.4 Create `native/libvexart/src/layout/writeback.rs` with `write_layout()` that reads `taffy::Tree::layout(node)` computed values and writes `PositionedCommand` structs to the caller output buffer. Respect `out_cap`. Per design §4.
...
- [x] [ATOMIC] 9.3 Modify `packages/engine/src/ffi/gpu-renderer-backend.ts` (78KB — largest TS change): rework to emit to packed graph buffer, call `vexart_paint_dispatch` once per frame instead of per-operation bridge calls. Split into sub-tasks:
  ...
  - [x] 9.3c Replace image upload/download with `vexart_paint_upload_image` / `vexart_paint_remove_image`. **PARTIAL completion (2026-04-17): flushImages/flushTransformedImages/flushGlyphs migrated to vexart_paint_dispatch cmd_kinds 9/10 and DEC-011 no-op; getImage() uses vexartUploadImage() with WeakMap cache. HOWEVER, the direct `from "./wgpu-canvas-bridge"` import was replaced with `from "./gpu-stub"` (a 86-LOC passthrough that re-exports from wgpu-canvas-bridge transitively). The real decoupling of ~15 renderWgpuCanvasTarget*Layer calls, filterWgpuCanvasImageBackdrop, maskWgpuCanvasImageRoundedRect*, and target-lifecycle helpers is deferred to Slice 11B tasks 11B.0a-0f (expanded scope per design §17.8). Slice 11B grep gates now depend on that migration completing.**
...
- [x] [ATOMIC] 9.11 Modify `packages/engine/src/output/kitty-shm-native.ts`: rewire `bun:ffi` loader from `libkitty-shm-helper` to `libvexart`'s `vexart_kitty_shm_*` exports. Signature-compatible. Per design §11.
...
**SCOPE EXPANDED (2026-04-17, design §17.8):** Apply #6 Slice 9 partial completion left `gpu-renderer-backend.ts` importing ~25 symbols from `./gpu-stub` (which transitively re-exports from `./wgpu-canvas-bridge`). 11B now absorbs the remaining migration burden before the delete can execute.

- [ ] 11B.0a [ATOMIC] Migrate the ~15 `renderWgpuCanvasTarget*Layer(...)` flush calls in `gpu-renderer-backend.ts` to `vexart_paint_dispatch` graph buffer writes, one cmd_kind per call. Mapping per design §17.8:
      rects → 0, shape_rects → 1, shape_rect_corners → 2, circles → 3, polygons → 4, beziers → 5, glows → 6, nebulas → 7, starfields → 8, images → 9, transformed_images → 10, linear_gradients → 12, radial_gradients → 13. Glyphs are skipped (DEC-011 no-op). Per design §17.8.
- [ ] 11B.0b [ATOMIC] Migrate `filterWgpuCanvasImageBackdrop` to cmd_kinds 15 (backdrop_blur) + 16 (backdrop_filter) graph buffer commands in `gpu-renderer-backend.ts`. Per design §17.8.
- [ ] 11B.0c [ATOMIC] Migrate `maskWgpuCanvasImageRoundedRect` and `maskWgpuCanvasImageRoundedRectCorners` to cmd_kind 17 (image_mask) in `gpu-renderer-backend.ts`. Single shader handles both uniform and per-corner modes. Per design §17.8.
- [ ] 11B.0d [ATOMIC] Replace target-lifecycle helpers (`createWgpuCanvasTarget`, `destroyWgpuCanvasTarget`, `beginWgpuCanvasTargetLayer`, `endWgpuCanvasTargetLayer`, `readbackWgpuCanvasTargetRGBA`, `copyWgpuCanvasTargetRegionToImage`) with vexart equivalents (`vexart_composite_readback_rgba`, `vexart_composite_readback_region_rgba`). If any helper has no vexart counterpart, flag for a small libvexart FFI extension and bump EXPECTED_BRIDGE_VERSION to 0x00020001. Per design §17.8.
- [ ] 11B.0e [ATOMIC] Remove inline `copyGpuTargetRegionToImage` and `createEmptyGpuImage` helpers (lines 83-105 of gpu-renderer-backend.ts) — replace call sites with vexart equivalents from 11B.0d or drop if dead. Per design §17.8.
- [ ] 11B.0f Remove the `import { ... } from "./gpu-stub"` block entirely from `gpu-renderer-backend.ts`. After this, `rg "from ['\"].*gpu-stub['\"]" packages/engine/src/ | grep -v "gpu-stub.ts:"` MUST return 0 lines. Per design §17.8.
