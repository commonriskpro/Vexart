...
- [ ] [ATOMIC] 5.2 Create `native/libvexart/src/paint/instances.rs` with `#[repr(C)]` `Pod + Zeroable` instance structs for each pipeline: `RectInstance`, `CircleInstance`, `LineInstance`, `BezierInstance`, `GradientInstance`, `GlowInstance`, `ShadowInstance`, `BlurSourceInstance`, `FilterInstance`, `BlendInstance`, `ImageInstance`. Per design §4.
...
- [ ] 10.8 Create `native/libvexart/src/paint/instances.rs` unit tests: `Pod + Zeroable` round-trips through `bytemuck::cast_slice`. Per design §13.1.
