...
│  — Public mount/terminal/hooks/types/debug; internal reconciler           │
...
1. **Reconciler Singleton Invariant**: The root `vexart` barrel and `@vexart/engine` must resolve to exactly one SolidJS universal reconciler instance (`createRenderer<TGENode>`). Published consumer JSX resolves through `vexart/jsx-runtime`; the workspace compiler may use `@vexart/engine/jsx-runtime` as compiler-only wiring to that same instance. Creating multiple reconcilers fragments node tracking, breaks fine-grained reactivity, and leaks retained layout nodes.
...
3. **Engine API Boundary Invariant**: The published `@vexart/engine`/`vexart/engine` surface contains `mount`, `createTerminal`, user-facing hooks, public types, and supported debug controls. Raw node construction, reconciler helpers, manual render-loop construction, layout, native/renderer backends, FFI bindings, and diagnostic state/culling helpers are internal. `vexart/jsx-runtime` is the published JSX runtime; `@vexart/engine/jsx-runtime` is compiler-only workspace wiring, and `@vexart/engine/internal` is workspace-only for maintainers and tests.
4. **Alpha Representation Invariant**: GPU render targets operate in **premultiplied alpha** ($[R \cdot A, G \cdot A, B \cdot A, A]$) to preserve linear blending, filtering, and blur correctness without dark edge fringing. Image assets, canvas buffers, and CPU host readback buffers operate in **straight alpha** ($[R, G, B, A]$). Full and regional readback normalize to straight alpha at the Rust host boundary; fully opaque packed buffers can be borrowed without conversion. GPU-to-GPU captures use the `image_unpremultiply` pipeline when producing straight-alpha image assets. Never unpremultiply a host readback a second time.
5. **ARM64 FFI Parameter Limit Invariant**: Foreign function calls between Bun (`bun:ffi`) and native Rust (`libvexart`) must never exceed **8 register arguments**. Any operation requiring more parameters must serialize parameters into a contiguous binary packed struct passed by pointer (`*const u8` or `*mut u8`).
6. **Terminal Accessibility Invariant**: Terminal emulators lack browser DOM trees, HTML elements, and web accessibility APIs (no `role="button"`, no `aria-expanded`). Accessibility in Vexart is implemented through explicit focus trees (`FocusScope`), circular tab cycling, Vim navigation keymaps (`h`/`j`/`k`/`l`), and focus traps for modal overlays.
...
### Pixel resource lifecycle

- **Images:** registration owns the cache reference. Each node acquires a separate native reference; removal, source replacement and loop teardown release it. LRU eviction and `clearImageCache()` release cache ownership only, not a live node's texture.
- **Decode publication:** source revisions cancel obsolete node subscriptions. Clearing the cache invalidates the request generation, so a late decode cannot repopulate it. Node references are acquired synchronously before another decode can evict the cache entry.
- **Targets:** allocate a replacement before destroying the previous target. Failed allocation leaves the previous record valid; successful replacement can temporarily require memory for both. Withdraw queued presentation metadata before freeing its target.
- **SHM:** regular Kitty and tmux share the active/latest-pending controller. A published segment is retained until the terminal unlinks its name, or the session explicitly cancels/fails. Regular Kitty has no elapsed-time permission to recycle a segment. The synchronous native ring refuses a full set of unconsumed transfers instead of unlinking one; it does not silently switch transports.
- **Wire representation:** raw/zlib bytes carry their encoding metadata together, even when compression increases size. Public full/region readback is straight RGBA; GPU composition remains premultiplied. Eight-bit storage can introduce rounding when converting back.

The image-retain and owned-SHM bridge symbols require rebuilding `libvexart` together with the TypeScript engine. The public JSX/reconciler surface is unchanged.
...
| `packages/engine` | `@vexart/engine` | Tier 4: Public `mount`, `createTerminal`, user-facing hooks/types, and debug controls; internal universal reconciler, Flexily layout, render graph, frame scheduler, terminal parser, and `bun:ffi` bridge. | Active |
