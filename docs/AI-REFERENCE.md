...
| **13**| Passing cell coordinates to DevTools MCP mouse tools | Assuming terminal coordinates use legacy cell columns/rows. | Clicks land near the top-left corner instead of the target element. | With SGR-Pixel (mode 1016), DevTools `vexart_click` takes **pixel coordinates** (`x`, `y`) matching screenshot dimensions directly. |
