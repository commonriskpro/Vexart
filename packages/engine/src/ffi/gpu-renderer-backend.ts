  ...
  const renderFrame = (
    ...
    try {
      for (const op of ctx.graph.ops) {
        ...
        if (op.kind === "text") {
          // ── MSDF path: delegate full text rendering to Rust ──
          // Rust handles: font resolution → MSDF glyph generation → atlas packing →
          // GPU upload → glyph instance building → dispatch.
          // Falls back to bitmap atlas path if MSDF dispatch fails.
          let usedMsdfPath = false
          try {
            const { symbols } = openVexartLibrary()
            const textX = Math.round(op.command.x) - ctx.offsetX
            const textY = Math.round(op.command.y) - ctx.offsetY

            // Resolve font families for Rust fontdb query.
            const fontDesc = getFont(op.inputs.fontId)
            const familiesJson = JSON.stringify([fontDesc.family, "sans-serif"])
            const familiesBytes = new TextEncoder().encode(familiesJson)

            // Pack color as RGBA u32.
            const colorRgba = (
              ((op.command.color[0] & 0xFF) << 24) |
              ((op.command.color[1] & 0xFF) << 16) |
              ((op.command.color[2] & 0xFF) << 8) |
              (op.command.color[3] & 0xFF)
            ) >>> 0

            // Build packed params: 28 bytes fixed header + families JSON.
            const headerSize = 28
            const paramsSize = headerSize + familiesBytes.length
            const paramsBuf = new ArrayBuffer(paramsSize)
            const paramsView = new DataView(paramsBuf)
            paramsView.setFloat32(0, textX, true)        // x
            paramsView.setFloat32(4, textY, true)        // y
            paramsView.setFloat32(8, op.inputs.fontSize, true) // font_size
            paramsView.setFloat32(12, op.inputs.lineHeight, true) // line_height
            paramsView.setFloat32(16, op.inputs.maxWidth || 0, true) // max_width
            paramsView.setUint32(20, colorRgba, true)    // color_rgba
            paramsView.setUint16(24, fontDesc.weight ?? 400, true) // weight
            paramsView.setUint16(26, fontDesc.style === "italic" ? 1 : 0, true) // flags (bit 0 = italic)
            // Append families JSON after header.
            const paramsU8 = new Uint8Array(paramsBuf)
            paramsU8.set(familiesBytes, headerSize)

            // Encode text.
            const textBytes = new TextEncoder().encode(op.inputs.text)

            const rc = symbols.vexart_font_render_text(
              vctx,
              targetHandle,
              ptr(textBytes), textBytes.length,
              ptr(paramsU8), paramsSize,
              ptr(_flushStatsBuf),
            ) as number
            ...
            if (rc === 0) {
              ...
              first = false
              targetMutationVersion += 1
              // Mark the text bounds as dirty.
              markDirty(
                Math.max(0, textX),
                Math.max(0, textY),
                Math.min(ctx.target.width, textX + op.command.width),
                Math.min(ctx.target.height, textY + op.command.height),
              )
            }
          } catch {
            // MSDF path unavailable or failed — fall through to bitmap.
          }
          ...
          // ── Bitmap atlas fallback path ──
          // Used when MSDF is unavailable or for backward compatibility.
