...
describe("native image assets", () => {
  ...
  test("accepts dynamic context handle while preserving backward compatibility", () => {
    ...
    const key = `test-image-ctx-${Date.now()}-${Math.random()}.rgba`

    const handle = nativeImageAssetRegister({ key, data, width: 2, height: 2, ctx: 1n }, 1n)
    expect(handle).not.toBeNull()
    expect(nativeImageAssetTouch(handle!, 0n, 1n)).toBe(true)
    expect(nativeImageAssetRelease(handle!, 1n)).toBe(true)
  })
