...
export type LayerSlot = { key: string; z: number; cmdIndices: number[] }
export type LayerBoundary = { path: string; nodeId: number; z: number; isScroll: boolean; hasBg: boolean; insideScroll: boolean }
...
export function findLayerBoundaries(node: TGENode, path: string, result: LayerBoundary[], nextZ: () => number, shouldPromoteLayer: (node: TGENode) => boolean, insideScroll = false) {
  ...
  if (node.props.layer === true || isInteractionLayer) {
    result.push({ path, nodeId: node.id, z: nextZ(), isScroll, hasBg: node.props.backgroundColor !== undefined, insideScroll })
  }
...
export function claimScissorCommands(commands: RenderCommand[]) {
  ...
  for (let i = 0; i < commands.length; i++) if (commands[i].type === CMD.SCISSOR_START) scissorStarts.push(i)
