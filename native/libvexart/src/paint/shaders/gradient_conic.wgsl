// gradient_conic.wgsl
// Conic (angular) gradient — NEW GPU pipeline (Slice 5b, cmd_kind = 14).
// Algorithm: compute atan2(uv.y - 0.5, uv.x - 0.5) normalised to [0,1],
// then mix from_color → to_color along that angle offset by start_angle.
// Optional corner-radius mask (uniform, uses box_w/box_h/radius).
...
fn fs_main(in: VSOut) -> @location(0) vec4<f32> {
  ...
  // Compute angle from center [0,1] UV, offset by start_angle, normalise to [0,1].
  let dx = in.uv.x - 0.5;
  let dy = in.uv.y - 0.5;
  let angle = atan2(dy, dx) - in.start_angle;
