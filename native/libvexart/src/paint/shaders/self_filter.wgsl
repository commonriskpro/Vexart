...
// Applies the same 7-op filter chain as backdrop_filter.wgsl, but reads from
// the element's own paint output texture rather than the background behind it.
//
// Pipeline: element content is first rendered to an isolated temp target,
// then this shader reads that target and applies the filter chain.
//
// Filter chain order (matches backdrop_filter.wgsl for consistency):
...
fn vs_main(
  ...
  // Source is the element's own paint output (not the scene backdrop),
  // so UV maps [0,1] directly over the element's isolated texture region.
  out.uv = vec2<f32>(
